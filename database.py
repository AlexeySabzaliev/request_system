...
# Для локального запуска или Render.com
DATABASE_URL = os.environ.get("DATABASE_URL", "postgresql://postgres:root@localhost:5432/request_system")
...
def init_db():
    ...
    if cur.fetchone()['count'] == 0:
        ...
        cur.execute("""
            ...
            ('Иван Иванов', 'executor@test.com', %s, (SELECT role_id FROM roles WHERE role_name='Исполнитель')),
            ('Петр Петров', 'user@test.com', %s, (SELECT role_id FROM roles WHERE role_name='Заявитель'))
